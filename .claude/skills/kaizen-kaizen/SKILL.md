---
name: kaizen-kaizen
description: Apply Kaizen principles (iterative improvement, error-proofing, YAGNI) to a specific change. Explicit-invoke only.
---
...
<Good>
```typescript
// Final iteration: works, clear, validated
...
Reached via three commits — first make it work, then refactor for clarity, then add validation — each step complete, tested, and working before the next.
</Good>
...
- **`/kaizen-why`**: Root cause analysis (5 Whys)
- **`/kaizen-cause-and-effect`**: Multi-factor analysis (Fishbone)
- **`/kaizen-plan-do-check-act`**: Iterative improvement cycles
- **`/kaizen-analyse-problem`**: Comprehensive documentation (A3)
- **`/kaizen-analyse`**: Smart method selection (Gemba/VSM/Muda)
