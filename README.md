...
**20 passing tests pin the budget governor's *measurement*** (run `pytest` from
...
| `impag-budget-check.py` | Stop a long run before the context cliff | `PostToolUse` on Bash; exact token read, hard-stop at 130k | Wired via `settings.json`; fires at a `git commit` past 130k | Interrupts mid-run at the threshold | 20 passing tests |
